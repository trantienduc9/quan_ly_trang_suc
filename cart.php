...
<div class="col-lg-12">	
    			...
    			<li class="list-group-item" data-id="<?php echo $row['id'] ?>" data-price="<?php echo $row['price'] ?>">
    				<div class="d-flex w-100">
    					...
    					<div class="detail-field">
    						<p>Tên sản phẩm: <b><?php echo $row['pname'] ?></b></p>
    						<p>Giá: <b><?php echo number_format($row['price'],3) ?></b></p>
    						<p>Kích thước: <b><?php echo $size ?></b></p>
    						<p>Màu sắc: <b><?php echo $colour ?></b></p>
    					...
    					<div class="amount-field">
    						<b class="amount"><?php echo number_format($row['qty']*$row['price'],3) ?></b>
    					</div>
    ...
    <div class="col-md-4">
    	<div class="card mb-4">
    		<div class="card-header bg-primary text-white"><b>Tổng thanh toán</b></div>
    		<div class="card-body">
    			<h4 class="text-right"><b id="tamount"><?php echo number_format($total,2) ?></b></h4>
    		</div>
    	</div>
    	<button class="btn btn-block btn-primary" id="checkout" type="button">Thủ tục thanh toán</button>
    </div>
</div>
</div>
<script>
	$('.btn-minus').click(function(){
     ...
     $('.rem_item').click(function(){
     	_conf("Bạn có chắc chắn xóa giỏ hàng này không?","delete_cart",[$(this).attr('data-id')])
     })
     function delete_cart($id){
     	...
     	$.ajax({
        	...
        	success:function(resp){
        		if(resp == 1){
            		alert_toast("Đã xóa sản phẩm khỏi cửa hàng","success");
